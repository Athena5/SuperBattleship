...
var SuperBattleship = function (custom_options) {
    ...
    var setupBoard = function () {
		...
		var pickInitialPosition = function(area, key, fleet, size) {
		    ...
		    var directions = [SBConstants.NORTH,
		    				  SBConstants.SOUTH,
		    				  SBConstants.EAST,
		    				  SBConstants.WEST];
		...
		options.fleet.forEach(function (si) {
	    	...
	    	var pos = pickInitialPosition(p1_init_area, p1_key, player_one_fleet, si.size);
	    	player_one_fleet.push(new Ship(si.name, si.size, pos, p1_key, that));
	    
	    	pos = pickInitialPosition(p2_init_area, p2_key, player_two_fleet, si.size);
    ...
    var switchTurns = function () {
		if (status == SBConstants.REGISTERING_PLAYERS || status == SBConstants.FINISHED) {
		    return;
		}
    ...
    var rotateShip = function(key, ship, is_cw) {
		...
		var new_dir = is_cw ? SBConstants.nextDirCW(pos.direction) : SBConstants.nextDirCCW(pos.direction);
    ...
    this.startGame = function() {
		if (status != SBConstants.REGISTERING_PLAYERS || p1_key == null || p2_key == null) {
	    	return false;
		}
    ...
    this.lookupSegmentIndex = function (gk, x,y) {
    	...
    	this.getSegmentState = function(gk, seg_idx) {
    		...
    		this.getStatus = function() {
				return (segment_status.find(function (s) {
						return s == SBConstants.OK;
					}) == undefined ? SBConstants.DEAD : SBConstants.ALIVE);
    	}
};